...
pub fn run(input: String) -> Result<(i64, i64), &'static str> {
    ...
    for line in lines {
        ...
        match *command {
            "nop" => machine.commands.push(Command{inst: CommandType::Nop, size}),
            "acc" => machine.commands.push(Command{inst: CommandType::Acc, size}),
            "jmp" => machine.commands.push(Command{inst: CommandType::Jmp, size}),
        ...
        loop {
            ...
            match command {
                Some(c) => match c.inst {
                    ...
                    CommandType::Acc => {self.acc += c.size;  self.pos += 1},
                    // Move to the command indicated by size relative to the current position
                    CommandType::Jmp => self.pos = (self.pos as isize).checked_add(c.size).unwrap() as usize,
                }
